...
Model Control::m_quadModel;// = QuadModel(1,1);
...
void Control::init(string font, int size, int screenWidth, int screenHeight)
{
    ...
    ModelManager::buildQuadModel(&m_quadModel, glm::vec3(0, 0, 0), glm::vec3(1, 1, 1));
...
bool Control::update(GameInput& gameInput)
{
    int x = gameInput.mousePosition.x;
    int y = m_screenHeight - gameInput.mousePosition.y;
...
bool Control::update(GameInput& gameInput, unsigned int& groupFlag)
{
    bool flag = update(gameInput);
    if(flag)
    {
