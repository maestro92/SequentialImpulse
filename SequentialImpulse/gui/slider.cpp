...
bool Slider::update(GameInput& state)
{
    Control::update(state);

    int x = state.mousePosition.x;
    int y = state.mousePosition.y;

    if(m_isInside == true)
    {
        if(state.mouseButtons[LEFT].endedDown)
        {
            m_dragging = true;
        }

        if(state.mouseButtons[RIGHT].endedDown)
        {
...
bool Slider::update(GameInput& state, unsigned int& groupFlag)
{
...
bool Slider::update1(GameInput& state, unsigned int& groupFlag)
{
    if(groupFlag & ( 1 << m_id) || (groupFlag==0) )
    {
        bool flag = update(state);
